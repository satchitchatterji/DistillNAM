...
class NAM(torch.nn.Module):
    ...
    def __init__(
        ...
        self._bias = torch.nn.Parameter(data=torch.zeros(self.num_outputs))

    def calc_outputs(self, inputs: torch.Tensor) -> Sequence[torch.Tensor]:
        """Returns the outputs computed by each feature net."""
        return [self.feature_nns[i](inputs[:, i]) for i in range(self.num_inputs)]

    def forward(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        individual_outputs = self.calc_outputs(inputs)
        # print(len(individual_outputs), individual_outputs[0].shape)

        conc_out = torch.stack(individual_outputs)
        # print(f"{conc_out.shape=}")

        dropout_out = self.dropout_layer(conc_out)

        out = torch.sum(dropout_out, dim=0)
        # print(out.shape, self._bias.shape)
        return out + self._bias, dropout_out # out: 1024x10
